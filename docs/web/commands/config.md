...
The config file lives outside `~/.stac-man/` (which is what `sm update` overwrites), so it survives every update. To remove it, delete `~/.config/stac-man/` or run the [uninstaller](/get-started/install#uninstall) with `--purge`.

## See also

- [Get Started → Install](/get-started/install) — including the **Uninstall** section.
