...
[`sm absorb`](/v1/commands/absorb) wraps [`git-absorb`](https://github.com/tummychow/git-absorb), which matches each hunk to the commit that introduced the surrounding code:
...
Resolve, `git add`, [`sm continue`](/v1/commands/continue).

## See also

- [`sm absorb`](/v1/commands/absorb) — flag reference.
- [`sm modify`](/v1/commands/modify) — when the fix belongs on the current tip.
