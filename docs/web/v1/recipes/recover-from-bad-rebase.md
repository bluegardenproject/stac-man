...
After any manual `git reset` of a tracked branch, run [`sm doctor`](/v1/commands/doctor) and then [`sm restack`](/v1/commands/restack) so `sm`'s view catches up.
...
- **Don't `git push --force`.** Use [`sm submit`](/v1/commands/submit) so the force becomes `--force-with-lease`.

## See also

- [`sm continue` / `sm abort`](/v1/commands/continue).
- [`sm undo`](/v1/commands/undo).
- [`sm doctor`](/v1/commands/doctor).
- [Concepts → Restack](/v1/concepts/restack#pause-and-resume).
