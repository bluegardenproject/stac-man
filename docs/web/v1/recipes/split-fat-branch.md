...
You started with one branch, kept committing, and ended up with seven commits that really should be three reviewable PRs. [`sm split`](/v1/commands/split) is the verb.
...
Don't like the result? [`sm undo`](/v1/commands/undo) rewinds the split:
...
- [`sm split`](/v1/commands/split) — flag reference.
- [`sm fold`](/v1/commands/fold) — the inverse direction.
