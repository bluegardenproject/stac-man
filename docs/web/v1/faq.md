...
See [Concepts → Parent metadata](/v1/concepts/parent-metadata) for details.
...
2. **The PR base graph on GitHub already encodes the same shape.** [`sm get <PR>`](/v1/commands/get) reconstructs local metadata from a PR chain.
